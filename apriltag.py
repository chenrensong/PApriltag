...
class myapriltag(object):
    def __init__(self):
        self.tagfamily = None
        self.tagdetector = None

    def create_detector(self, family='tag36h11', sigma=0.8, nthread=1, debug=False, minarea=400,
                        thresholding='adaptive', downsampling=False):
    ...
    def detect(self, frame):
        ...
        if (self._thresholding == 'canny'):
            ...
            img = np.array(cv2.adaptiveThreshold(img, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, 9, 5),
                           dtype='uint8')
